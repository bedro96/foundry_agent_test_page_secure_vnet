 ...
 * 로그인 페이지.
 *
 * 사용자가 이메일과 비밀번호로 인증할 수 있는 전체 화면 로그인 폼을 렌더링합니다.
 * 좌측에 공장 배경 이미지, 우측에 로그인 폼과 시스템 소개를 표시합니다.
 * @module login/page
 */
 ...
 * 로그인 페이지 컴포넌트.
 *
 * 백엔드 인증 API에 자격 증명을 전송하고,
 * 성공 시 `/chat` 페이지로 리다이렉트합니다.
 */
export default function LoginPage() {
   ...
   * 로그인 폼 제출 핸들러.
   *
   * 백엔드 로그인 API를 호출하고 성공 시 채팅 페이지로 이동합니다.
   */
  ...
  return (
    <main className="fixed inset-0 z-50 flex bg-background text-foreground">
      ...
      <section className="relative hidden md:flex md:w-1/2">
        ...
        <div className="absolute inset-x-0 bottom-0 p-10 text-white">
          ...
          <p className="mt-4 max-w-md text-sm leading-6 text-white/80 lg:text-base">
            Monitor production insights, investigate issues faster, and collaborate with AI-powered
            support from one workspace.
          </p>
        </div>
      </section>
          ...
          <div className="max-w-xl space-y-6 text-center sm:text-left">
            ...
            <div className="space-y-4">
              ...
              <p className="text-base leading-7 text-muted-foreground lg:text-lg">
                Sign in to access live chat, source-backed answers, and production support
                workflows tailored for manufacturing teams.
              </p>
            </div>
            {/* 기능 소개 카드 그리드 */}
            <div className="grid gap-3 text-left text-sm text-muted-foreground sm:grid-cols-2">
              <div className="rounded-2xl border bg-card p-4">
                <p className="font-medium text-foreground">Search-backed answers</p>
                <p className="mt-1 leading-6">
                  Pull in research, operational context, and citations while the assistant streams
                  responses.
                </p>
              </div>
              <div className="rounded-2xl border bg-card p-4">
                ...
                <p className="mt-1 leading-6">
                  Keep teams aligned with fast access to troubleshooting, process summaries, and
                  MCP tooling.
                </p>
              </div>
            </div>
